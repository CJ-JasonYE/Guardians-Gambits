...
public class Hovl_Laser2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (laserPS != null && UpdateSaver == false)
        {
            ...
            if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, MaxLength))
            {
                ...
                //Change Box collider size when collide with objects
                ...
                boxCollider.center = new Vector3(0, 0, hit.distance - boxCollider.size.z / 2);
            ...
            else
            {
                ...
                //Change Box collider size when no collide with objects
                Vector3 colliderSize = boxCollider.size;
                //Debug.Log("size"+colliderSize);
                colliderSize.z = MaxLength;
                boxCollider.size = colliderSize;
                colliderSize = boxCollider.size;

                boxCollider.center = new Vector3(0, 0, MaxLength - boxCollider.size.z/2);
                //Debug.Log("center" + boxCollider.center);
            }          
        }
        //PressBlockChange();
    ...
    private void PressBlockChange()
    {
        if (Input.GetKeyDown(KeyCode.Space))
        {
            boxCollider.center= new Vector3(10,10,10);
        }
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            other.GetComponent<Enemy>().TakeDamage(laserDamage);
        }
    }
